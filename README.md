...
この構成では **Matrix(Synapse) サーバーは同梱していません**。
そのため**永続チャット**（ルーム・ダイレクトメッセージ）は利用できませんが、
バブル/会議エリア内での**近接テキストチャット**は `ENABLE_CHAT=true`（既定）で利用できます。

- **Jitsi（会議エリア）・バブル通話（近接の音声/ビデオ）は Matrix とは独立**しており、
  Matrix が無くてもそのまま利用できます。
- 永続チャットが必要になった場合は、別途 Matrix(Synapse) サーバーを立て、`play` の
  `MATRIX_API_URI` / `MATRIX_PUBLIC_URI` / `MATRIX_DOMAIN` を設定してください。

> **メモ:** Synapse を同梱しないぶん RAM 消費が抑えられます（目安 −0.3〜0.5GB）。
...
- **`DNS problem: NXDOMAIN looking up A for <ドメイン>`** とログに出る
  → そのドメインの **A レコード（IPv6 を使うなら AAAA レコード）が未作成**です。
  DNS（ドメイン管理画面）で `DOMAIN` の値（例 `wa.example.com`）をサーバーのグローバル IP に
  向けるレコードを作成し、`dig +short <ドメイン>` でこのサーバーの IP が返ることを確認してください。
  反映には数分〜数時間かかる場合があります。
  ※ **Cloudflare Tunnel（公開方法 C）を使う場合**は TLS を Cloudflare 側で終端するため、
  Let's Encrypt は不要です。`.env` の `COMPOSE_FILE` に `docker-compose.cloudflare.yaml` を
  含めて運用してください（このオーバーレイ無しだと Traefik が証明書取得を試み続けて上記エラーを出します）。
- **ビデオ・音声がつながらない**
