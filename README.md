...
- **チャットが使えない / `synapse` が再起動を繰り返す・`getaddrinfo ENOTFOUND synapse`**
  → `./logs.sh` で `synapse` のログを確認。署名鍵生成で失敗していた問題は修正済みなので、
  `git pull` で最新化したうえで `docker compose up -d` で再作成してください。
- **ビデオ・音声がつながらない**
